...
/**
 * Модуль статистики (данных из второй базы)
...
class CommunicationStat extends Model
{
    ...
    public static function  value_tariff($serial_number, $date_start = null, $details = 'month'){
        if(is_null($date_start)){
            $date_start = date('Y-m-d H:i:s', strtotime('-3 month'));
        }
        $val_tariff =  Yii::$app->db_ext->createCommand('select MeterId, SerialNumber, DT, SUM(IF(TariffId=1,Val,null)) as Val_1,
	SUM(IF(TariffId=2,Val,null)) as Val_2, SUM(IF(TariffId=3,Val,null)) as Val_3
	from (SELECT `values`.MeterId as Meterid, `meters`.SerialNumber  as SerialNumber,`values`.DT as DT,`values`.TariffId as Tariffid,
	`values`.val as Val FROM `values`,`meters` where
	`values`.MeterId=`meters`.MeterId and `meters`.SerialNumber=:serial_number) as D
	group by DT order by DT asc', ['serial_number' => $serial_number])->queryAll();
        return $val_tariff;
    }
  ...
  (select meters.serialnumber,mx,meters.meterid from (select max(f.dat) as mx,f.meterid  from
    ...
    public static function recentTestimonies1($serial_number){
        return Yii::$app->db_ext->createCommand('select exp1, expr1, serialnumber from (select `values`.val as exp1,b.mx as expr1,serialnumber from
  (select meters.serialnumber,mx,meters.meterid from (select max(f.dat) as mx,f.meterid  from
    (select `values`.meterid as meterid, `values`.val as num, `values`.dt as dat from  `values`) as f group by f.meterid) as d inner join `meters`
      on d.meterid=meters.meterid) as b inner join `values` on b.meterid=`values`.meterid and b.mx=`values`.dt where serialnumber=:serial_number) as yy', ['serial_number' => $serial_number])->queryAll();
    }

    /**
     * Подготавливаем (детализируем) данные для отрисовки в графике - реализовано только для месяца
     * @param $data
     * @param $type
     * @return array
     */
